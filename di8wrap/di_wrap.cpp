...
HRESULT WINAPI DetourDirectInput8Create(
	HINSTANCE inst_handle,
	DWORD version,
	REFIID r_iid,
	DI_HID_WrapperBase ** out_wrapper,
	LPUNKNOWN p_unk) {
	
	if (r_iid == GUID{ 0xBF798030,0x483A,0x4DA2,{0xAA,0x99,0x5D,0x64,0xED,0x36,0x97,0x00} }) {
		DebugLog("[di8wrap] wrapping DirectInput8Create().\n");
	...
	DebugLog("[di8wrap] passthru DirectInput8Create().\n");
	return WrapperSystem::DirectInput8Create(inst_handle, version, r_iid, (LPVOID*)out_wrapper, p_unk);
}

bool WrapperSystem::Init(HANDLE mod_hnd)
{
	dinputDll = GetModuleHandleA("dinput8.dll");

	if (dinputDll > (HMODULE)31)
	{
		DIRECTINPUT8CREATEPROC TargetDirectInput8Create = (DIRECTINPUT8CREATEPROC)GetProcAddress(dinputDll, "DirectInput8Create");

		if (TargetDirectInput8Create) {
			MH_STATUS res = MH_CreateHook(TargetDirectInput8Create, &DetourDirectInput8Create, reinterpret_cast<LPVOID*>(&DirectInput8Create));
			if (res == MH_OK) res = MH_EnableHook(TargetDirectInput8Create);
			DebugLog("[di8wrap] hook DirectInput8Create with status = %d\n", res);
...
void WrapperSystem::Shutdown()
{
	...
	if (auto addr = GetProcAddress(dinputDll, "DirectInput8Create")) {
		MH_DisableHook(addr);
		MH_RemoveHook(addr);
	}
}
...
void WrapperSystem::InitHID(HWND& h_wnd)
{
	...
	hidInitialized = true;
	DebugLog("[di8wrap] InitHID Success\n");
}
...
LRESULT WrapperSystem::WndProcDo(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	if (uMsg == WM_INPUT)
	{
		...
		DWORD dwRet;
		UINT dwSize = 48;
		dwRet = GetRawInputData((HRAWINPUT)lParam, RID_INPUT, lpb, &dwSize, sizeof(RAWINPUTHEADER));
		raw = (RAWINPUT*)lpb;
...
LRESULT WrapperSystem::WindowHookFuncGetMessage(int nCode, WPARAM wParam, LPARAM lParam)
{
	...
	if (cwpData->message == WM_INPUT)
	{
		...
		if (raw->header.dwType == RIM_TYPEMOUSE && !raw->data.mouse.usFlags)
		{
			...
			mouseEvent.SetTime(lastEventTime);
			DI_HID_MouseEvent mouseEventWrap;
			if (!menu::isOpen && mouseEvent.Wrap(mouseEventWrap, type_axis_x) )
				PushEvent(mouseEventWrap);
...
HRESULT DI_HID_WrapperBase::CreateDevice(const GUID& r_guid, DI_HID_DeviceBase** di_device, LPUNKNOWN p_unk)
{
	...
	*di_device = AllocDevice();
	char szGuid[40] = { 0 };
	_snprintf_s(szGuid, 40, "%08lX-%04hX-%04hX-%02X%02X-%02X%02X%02X%02X%02X%02X",
			r_guid.Data1, r_guid.Data2, r_guid.Data3,
		r_guid.Data4[0], r_guid.Data4[1], r_guid.Data4[2], r_guid.Data4[3],
		r_guid.Data4[4], r_guid.Data4[5], r_guid.Data4[6], r_guid.Data4[7]);

	
	if (r_guid == GUID{ 0x6F1D2B60,0xD5A0,0x11CF,{0xBF,0xC7,0x44,0x45,0x53,0x54,0x00,0x00 }})
	{
		DebugLog("[di8wrap] CreateDevice(GUID_SysMouse)\n");
		isMouse = true;
	}

	else if (r_guid == GUID{ 0x6F1D2B61,0xD5A0,0x11CF,{0xBF,0xC7,0x44,0x45,0x53,0x54,0x00,0x00 } }) {
		DebugLog("[di8wrap] CreateDevice(GUID_SysKeyboard)\n");
	}
	else
		DebugLog("[di8wrap] CreateDevice(%s)\n", szGuid);
...
HRESULT DI_HID_DeviceBase::QueryInterface(const IID& r_iid, LPVOID* p_obj)
{
	//*p_obj = this;
	//return S_OK;
	return diDevice->QueryInterface(r_iid, p_obj);
}
...
HRESULT DI_HID_DeviceBase::SetProperty(const IID& r_iid, LPCDIPROPHEADER dip)
{
	...
	case 1:
		...
		return S_OK;
	case 2:
		absoluteAxis = ((LPDIPROPDWORD)dip)->dwData == DIPROPAXISMODE_ABS;
		return S_OK;
	}
	return DIERR_UNSUPPORTED;
}
...
HRESULT DI_HID_DeviceBase::SetDataFormat(LPCDIDATAFORMAT data_format)
{
	...
	{
#if 1
		auto &object_format = data_format->rgodf[i];
		DI_HID_Object objItr{};
		objItr.SetType(i);
		if (AlreadyInFormatList(objItr))
			continue;

		if (i < data_format->dwNumObjs - 1) {
			objItr.SetSize(data_format->rgodf[i + 1].dwOfs - object_format.dwOfs);
		}
		else {
			objItr.SetSize(data_format->dwObjSize - object_format.dwOfs);
		}
		AddToFormatList(objItr, object_format);

#else
		// Don't know how to fill objects.
		bool found = false;
		for (ObjectList::iterator objItr = objects.begin(); objItr != objects.end(); objItr++)
		{
			if (AlreadyInFormatList(*objItr))
